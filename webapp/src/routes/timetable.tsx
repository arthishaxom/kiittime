...
function TimetablePage() {
	...
	const [lastSeenAnnouncementId, setLastSeenAnnouncementIdState] = useState<
		number | null
	>(() => getLastSeenAnnouncementId());
	...
	useEffect(() => {
		if (
			announcement &&
			isAnnouncementUnseen(announcement.id, lastSeenAnnouncementId)
		) {
		...
		<div className="h-dvh bg-bg text-text flex flex-col">
			...
			<div ref={containerRef} className="flex-1 min-h-0 overflow-hidden">
				<motion.div
					...
					drag="x"
					dragConstraints={{
						left: -(DAYS.length - 1) * containerWidth,
						right: 0,
					}}
					dragElastic={0.15}
								...
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-text-muted text-xs">
											For a faster, native experience
										</p>
									</div>
								</div>
