export type Section = {
	id: number;
	section_name: string;
	year: number;
};

export type Session = {
	day: string;
	period_number: number;
	start_time: string;
	course_code: string;
	course_name: string | null;
	faculty_name: string;
	room_number: string;
	section: string;
};

export type Timetable = {
	sections_requested: string[];
	sessions: Session[];
};

export type Announcement = {
	id: number;
	title: string;
	body: string;
	link_label: string | null;
	link_url: string | null;
	created_at: string;
};

export type RollNumberMappingOut = {
	roll_no: string;
	academic_year: number;
	sections: Section[];
};

export function formatTime(time: string): string {
	const [hours, minutes] = time.split(":").map(Number);
	const date = new Date();
	date.setHours(hours, minutes, 0, 0);
	return new Intl.DateTimeFormat(undefined, {
		hour: "numeric",
		minute: "2-digit",
	}).format(date);
}

export async function fetchSections(baseUrl: string, year?: number): Promise<Section[]> {
	const url = new URL("/sections/", baseUrl);
	if (year !== undefined) {
		url.searchParams.set("year", String(year));
	}

	const res = await fetch(url);
	if (!res.ok) {
		throw new Error(`Failed to fetch sections: ${res.status}`);
	}
	return res.json();
}

export async function fetchTimetable(baseUrl: string, sectionIds: number[]): Promise<Timetable> {
	const url = new URL("/timetable/", baseUrl);
	for (const id of sectionIds) {
		url.searchParams.append("section_id", String(id));
	}

	const res = await fetch(url);
	if (!res.ok) {
		throw new Error(`Failed to fetch timetable: ${res.status}`);
	}
	return res.json();
}

export async function fetchCurrentAnnouncement(baseUrl: string): Promise<Announcement | null> {
	const url = new URL("/announcements/current", baseUrl);

	const res = await fetch(url);
	if (!res.ok) {
		throw new Error(`Failed to fetch current announcement: ${res.status}`);
	}
	return res.json();
}

export async function fetchRollNumberMapping(
	baseUrl: string,
	rollNo: string,
): Promise<RollNumberMappingOut> {
	const url = new URL(`/api/roll-numbers/${rollNo}`, baseUrl);
	const res = await fetch(url);
	if (!res.ok) {
		const detail = `Failed to fetch roll number mapping: ${res.status}`;
		let detailMessage = "";
		try {
			const body = await res.json();
			if (body?.detail) {
				detailMessage = body.detail;
			}
		} catch {}
		const error = new Error(detailMessage || detail);
		(error as any).status = res.status;
		(error as any).detail = detailMessage;
		throw error;
	}
	return res.json();
}

export async function sendOtp(
	baseUrl: string,
	rollNo: string,
	sectionIds: number[],
): Promise<{ status: string; message: string }> {
	const url = new URL("/api/auth/otp/send", baseUrl);
	const res = await fetch(url, {
		method: "POST",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({ roll_no: rollNo, section_ids: sectionIds }),
	});
	if (!res.ok) {
		let detailMessage = "";
		try {
			const body = await res.json();
			if (body?.detail) {
				detailMessage = body.detail;
			}
		} catch {}
		const error = new Error(
			detailMessage || `Failed to send OTP: ${res.status}`,
		);
		(error as any).status = res.status;
		throw error;
	}
	return res.json();
}

export async function verifyOtp(
	baseUrl: string,
	rollNo: string,
	otpCode: string,
): Promise<RollNumberMappingOut> {
	const url = new URL("/api/auth/otp/verify", baseUrl);
	const res = await fetch(url, {
		method: "POST",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({ roll_no: rollNo, otp_code: otpCode }),
	});
	if (!res.ok) {
		let detailMessage = "";
		try {
			const body = await res.json();
			if (body?.detail) {
				detailMessage = body.detail;
			}
		} catch {}
		const error = new Error(
			detailMessage || `Failed to verify OTP: ${res.status}`,
		);
		(error as any).status = res.status;
		throw error;
	}
	return res.json();
}
