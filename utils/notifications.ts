...
import { GroupedSchedule, ScheduleSlot } from '~/store/timetableState';
 ...
 */
const getNextOccurrence = (targetDay: number, targetHour: number, targetMinute: number): Date => {
  const now = new Date();
...
const scheduleClassNotification = async (
  ...
  const timeInfo = `${hour}:${minute} (${weekday}) (${classData.Time_Sort})`;
  ...
  const nextOccurrence = getNextOccurrence(weekday, notificationHour, notificationMinute);
  ...
  try {
    ...
    if (Platform.OS === 'android') {
      ...
      if (!hasExactAlarmPermission) {
        throw new Error('Exact alarm permission not granted. Notifications may not work reliably on Android 12+.')
      }
    }
    ...
    for (const day of days) {
      ...
      for (const classData of classes) {
        try {
          const notificationId = await scheduleClassNotification(classData, minutesBefore);
          notificationIds.push(notificationId);
          // console.log(`Scheduled notification ${notificationId} for ${classData.Subject}`);
        } catch (error) {
          console.error(`Failed to schedule notification for ${classData.Subject}:`, error);
        }
      }
    }
...
export const scheduleTestNotification = async (): Promise<string> => {
  try {
    ...
    console.log(`Will trigger at: ${new Date(trigger.timestamp!).toISOString()}`);
