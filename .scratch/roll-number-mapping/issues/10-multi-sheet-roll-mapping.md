...
- [ ] Update `POST /admin/roll-mappings/inspect` in FastAPI:
...
- [ ] Update `POST /admin/roll-mappings/upload` in FastAPI:
  - Accept an optional `sheet_name: str | None = Form(None)` form parameter.
  - If the file is Excel, parse the specified sheet. If CSV, ignore it.
- [ ] Update `admin-webapp`'s upload logic on the Roll Mappings tab:
...
- [ ] Add unit/integration tests to verify multi-sheet inspecting and uploading.
